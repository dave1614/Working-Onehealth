...
<script>
  ...
  function viewTestsAwaitingPathologistsComment (elem,evt,enter = true) {
    ...
    if(!enter){
      ...
      var url = "<?php echo site_url('onehealth/index/'.$addition.'/'.$second_addition.'/'.$third_addition. '/' . $fourth_addition .'/admin/awaiting'); ?>"
    }
        ...
        <?php } ?>
          ...
          <div class="row">
            <div class="col-sm-12">
              ...
              <div class="card" id="patients-awaiting-comments-card" style="<?php if(is_null($sixth_addition)){ echo "display: none"; } ?> ">
                <div class="card-header">
                  <button onclick="goBackFromPatientsAwaitingCommentsCard(this,event)" class="btn btn-warning">Go Back</button>
                  <h3 class="card-title">
                  <?php
                  if($sixth_addition == 'edit'){
                    echo "All Patients With Pathologist's Comments Entered";
                  }else if($sixth_addition == 'awaiting'){
                    echo "Patients Awaiting Pathologist's Comments";
                  }
                  ?>
                  </h3>
                </div>
                  ...
                  if(isset($rows)){
                    ...
                    if(is_array($initiation_codes)){
                      // var_dump($_GET);
                        ...
                        if($sixth_addition == 'edit'){
                          $response_arr['messages'] .= '<th>Comment Entered Time</th>';
                          
                        }else if($sixth_addition == 'awaiting'){
                          $response_arr['messages'] .= '<th>Verification Time</th>';
                        }
                        ...
                        foreach($initiation_codes as $row){
                          ...
                          if($this->input->post('edit')){
                            $tests_num = $this->onehealth_model->getTestsNumForPatientAwaitingPathologistsCommentEdit($health_facility_id,$initiation_code,$sub_dept_id);
                          }else{
                            $tests_num = $this->onehealth_model->getTestsNumForPatientAwaitingPathologistsComment($health_facility_id,$initiation_code,$sub_dept_id);
                          }
                          ...
                          if($sixth_addition == 'edit'){
                            $response_arr['messages'] .= '<tr onclick="loadTestsAndPatientInfo(this,event,'.$lab_id.',false,false)">';
                          }else if($sixth_addition == 'awaiting'){
                          ...
                          if($this->input->post('edit')){
                            $response_arr['messages'] .= '<td>'.$comments_entered_time.'</td>';
                          }else{
                            $response_arr['messages'] .= '<td>'.$verified_time.'</td>';
                          }
              ...
              <div class="card" id="main-card" style="<?php if($sixth_addition == 'edit' || $sixth_addition == 'awaiting'){ echo "display: none;"; }  ?>">
                <div class="card-header">
...
<script>
    $(document).ready(function() {
      <?php if($sixth_addition == "edit" || $sixth_addition == "awaiting"){ ?>
        
        $("html, body").animate({ scrollTop: $(document).height() }, 1000);
        
      <?php } ?>
      
      $("#filter-rows-form").submit(function (evt) {
        evt.preventDefault()
        var me  = $(this)
        ...
        <?php } else if($sixth_addition == "awaiting"){ ?>
          var url = "<?php echo site_url('onehealth/index/'.$addition.'/'.$second_addition.'/'.$third_addition. '/' . $fourth_addition .'/admin/awaiting'); ?>?lab_id="+lab_id+"&initiation_code="+initiation_code
        <?php } ?>

        window.location.assign(url)
      })

      $("#clear-filter-rows-form").click(function(evt){
        var me = $(this);
        $("#filter-rows-form input").val("")
