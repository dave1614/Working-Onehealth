      ...
      <script>
         function addCommas(nStr)
          ...
          function viewSmsBalance(elem){
            var url = "<?php echo site_url('onehealth/index/'.$addition.'/view_facility_sms_balance'); ?>";
            $(".spinner-overlay").show();
            $.ajax({
              type : "POST",
              dataType : "json",
              ...
              success : function (response) {
                ...
                if(response.success && response.messages != ""){
                  $("#main-card").hide("fast");
                  $("#sms-balance-card .card-body").html(response.messages);
                  $("#sms-balance-card #sms-balance-table").DataTable();
                  $("#sms-balance-card").show('fast');
                }else{
                  swal({
                    title: 'Ooops',
          ...
          function goBackFromSmsBalanceCard (elem,evt) {
            $("#main-card").show("fast");
            $("#sms-balance-card").hide('fast');
          }

          function creditSMSWithdrawal (elem,evt) {
            swal({
              title: 'Choose Action',
              text: "Do You Want To?",
              type: 'question',
              ...
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              confirmButtonText: 'Credit Your Sms Balance',
              cancelButtonText : "View SMS Credit History"
            }).then(function(){
              $("#enter-amount-to-credit-modal").modal("show");
            }, function(dismiss){
               if(dismiss == 'cancel'){
                var url = "<?php echo site_url('onehealth/index/'.$addition.'/view_sms_credit_history'); ?>";
                $(".spinner-overlay").show();
                $.ajax({
                  type : "POST",
                  dataType : "json",
                  ...
                  success : function (response) {
                    ...
                    if(response.success && response.messages != ""){
                      $("#main-card").hide("fast");
                      $("#sms-credit-history-card .card-body").html(response.messages);
                      $("#sms-credit-history-card #sms-credit-history-table").DataTable();
                      $("#sms-credit-history-card").show('fast');
                    }else{
                      swal({
                        title: 'Ooops',
                        text: "Sorry Something Went Wrong Please.",
          ...
          function goBackFromSmsCreditHistoryCard (elem,evt) {
            $("#main-card").show("fast");
            $("#sms-credit-history-card").hide('fast');
          }
        ...
        <div class="container-fluid">
          ...
          <div class="row ">
            <div class="col-sm-10">
              ...
              <div class="card" style="display: none;" id="sms-credit-history-card">
                <div class="card-header">
                  <button class="btn btn-warning" onclick="goBackFromSmsCreditHistoryCard(this,event)">Go Back</button>
                  <h3 class="card-title">SMS Credit History</h3>
                </div>
                <div class="card-body">
              ...
              <div class="card" id="main-card">
                ...
                <div class="card-body">
                  <button class="btn btn-primary" style="margin-top: 50px;" onclick="viewSmsBalance(this)">View SMS Balance</button>

                  <button class="btn btn-info" style="margin-top: 50px;" onclick="creditSMSWithdrawal(this)">Credit SMS Balance</button>
                </div>
              </div>



              <div class="card" style="display: none;" id="sms-balance-card">
                <div class="card-header">
                  <button class="btn btn-warning" onclick="goBackFromSmsBalanceCard(this,event)">Go Back</button>
                  <h3 class="card-title">Your SMS Balance</h3>
                </div>
                <div class="card-body">
      ...
      <div class="modal fade" data-backdrop="static" id="enter-amount-to-credit-modal" data-focus="true" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
        <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h3 class="modal-title">Amount To Credit Your Sms Balance</h3>
          ...
          <div class="modal-body" id="modal-body">
            ...
            <?php 
              $attr = array('id' => 'enter-amount-to-credit-form');
              echo form_open("",$attr);
            ?>
              
              <div class="form-group">
                <input type="number" class="form-control" id="amount" name="amount" min="200" step="any" required>
              </div>
      ...
      <script>
        $(document).ready(function () {
          ...
          $("#enter-amount-to-credit-form").submit(function (evt) {
            evt.preventDefault();
            var me = $(this);
            ...
            var url = "<?php echo site_url('onehealth/index/'. $addition .'/credit_sms_balance'); ?>";
            $(".spinner-overlay").show();
            $.ajax({
              type : "POST",
              dataType : "json",
              ...
              success : function (response) {
                ...
                if(response.success == true && response.url != ""){
                  ...
                  $.notify({
                  message : "Redirecting To Secure Payment Platform..."
                  },{
                    type : "success"  
                  });
                ...
                }else if(response.not_numeric){
                  swal({
                    title: 'Error',
                    text: "Amount Entered Is Invalid!",
                    type: 'error'                 
                  });
                }else if(response.too_small){
                  swal({
                    title: 'Error',
                    text: "Minimum Amount Is 200",
                    type: 'error'                 
                  });
                }else{
